...
function NewTomTomStoreLocator() {
  ...
  useEffect(() => {
    const loadTomTom = async () => {
      try {
        ...
        ;(window as any).tt = tt.default
        
        setTomtomLoaded(true)
      } catch (error) {
        console.error('Error loading TomTom SDK:', error)
        setError('Failed to load maps SDK. Please refresh the page and try again.')
      }
    }
  ...
  useEffect(() => {
    if (!apiKey || !tomtomLoaded || !mapRef.current) return
  ...
  const searchLocations = async () => {
    ...
    try {
      const tt = (window as any).tt
      const searchService = tt.services.search(apiKey)

      // First, geocode the search query
      const geocodeResponse = await searchService.geocode({
        query: searchQuery,
        limit: 1
      }).go()
      ...
      const searchLocation = {
        lat: geocodeResponse.results[0].position.lat,
        lng: geocodeResponse.results[0].position.lon,
        address: geocodeResponse.results[0].address.freeformAddress
      }

      // Search for Texas Roadhouse locations using POI search
      const poiResponse = await searchService.poiSearch({
        query: 'Texas Roadhouse',
        lat: searchLocation.lat,
        lon: searchLocation.lng,
      ...
      const locations: Location[] = poiResponse.results.map((result: any, index: number) => {
        return {
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <button
              ...
              disabled={loading || !mapLoaded || !tomtomLoaded}
...
export default NewTomTomStoreLocator
