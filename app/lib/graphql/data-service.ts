...
export async function fetchAllPosts() {
  try {
    ...
    const { data, error } = await client.query({
      ...
      context: {
        fetchOptions: {
          timeout: 5000, // Reduce timeout for faster failures
...
export async function fetchCategories() {
  try {
    ...
    console.log('📡 WORDPRESS RESPONSE:', { data, error })
    ...
    console.log('📂 CATEGORIES FOUND:', safeCategories.length, safeCategories)
...
// Fetch site SEO settings - temporarily simplified for build stability
export async function fetchSiteSEO() {
  try {
    // Temporarily return fallback data to avoid GraphQL SEO errors during build
    // The WordPress site may not have Yoast SEO plugin properly configured
    console.log('ℹ️  Using fallback SEO settings (WordPress Yoast SEO not available)')
    
    return {
      seo: {
        generalSettings: {
          title: 'Little Caesars Menu',
          description: 'Your ultimate guide to Little Caesars menu, prices, calories, deals, and dining experience.',
    ...
    console.error('Error in fetchSiteSEO fallback:', error)
    return {
      seo: null,
      error
    }
