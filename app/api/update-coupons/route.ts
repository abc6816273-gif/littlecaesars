...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Check if this is from Vercel cron (treat as force update)
    const userAgent = request.headers.get('user-agent') || '';
    const isCronJob = userAgent.includes('vercel-cron') || request.headers.get('x-vercel-cron');
    const shouldForce = force || isCronJob;
    
    console.log('📊 Request parameters:', { force, isCronJob, shouldForce, userAgent });
    ...
    return NextResponse.json({
      success: true,
      data: coupons,
      message: isCronJob ? 'Coupons updated by cron job successfully' : 
               shouldForce ? 'Coupons force updated successfully' : 
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (isCronJob) {
      console.log('🕐 Daily cron job triggered - Generating fresh coupons...');
    } else {
      console.log('👤 Manual trigger - Generating fresh coupons...');
    }
